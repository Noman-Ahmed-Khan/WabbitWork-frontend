...
/**
 * Handle accepting an invitation from an email link
 */
export default function AcceptInvitationView() {
  ...
  useEffect(() => {
    const processAcceptance = async () => {
      ...
      // But typically, accepting might require auth. 
      // If the API requires auth, the request will fail with 401 if they aren't logged in.
      // We will try anyway, and handle 401.
      if (!isAuthenticated) {
        // We could redirect them to login with a return URL, but for simplicity:
      ...
      <div className="w-full max-w-md">
        ...
        <Panel className="text-center overflow-hidden">
          {success ? (
            ...
            >
              <div className="w-20 h-20 rounded-full bg-success/10 flex items-center justify-center mx-auto mb-6 relative">
                <motion.div 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
              ...
              <h2 className="text-2xl font-bold mb-3 tracking-tight">Invitation Accepted!</h2>
            ...
            >
              ...
              <h2 className="text-2xl font-bold mb-3 tracking-tight">Could Not Accept</h2>
